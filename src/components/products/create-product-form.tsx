 ...
 * @description Form component for creating new catalog products with ImageKit banner, gallery, taxonomy, and SEO.
 */
...
import { Loader2, Sparkles, Image as ImageIcon, CheckCircle2, Flag } from "lucide-react"
...
import type { ProductStatus, CreateProductPayload, ProductSpecifications } from "@/types/product"
...
export function CreateProductForm({
  ...
  const [bannerAltText, setBannerAltText] = React.useState("")
  const [seoTitle, setSeoTitle] = React.useState("")
  const [seoDescription, setSeoDescription] = React.useState("")
  const [specifications, setSpecifications] = React.useState<ProductSpecifications | null>(null)
  ...
  }, [name, slug, description, categoryId, brandId, status, isFeatured, seoTitle, seoDescription, imageUrl, specifications, onFormChange])

  const handleNameChange = (val: string) => {
    setName(val)
    if (isAutoSlug) setSlug(generateSlug(val))
  }
    ...
    const payload: CreateProductPayload = {
      ...
      bannerImage: bannerImageUrl.trim()
        ? {
            ...
            thumbnailUrl: bannerImageUrl.trim().includes("imagekit.io") ? `${bannerImageUrl.trim()}?tr=w-300` : undefined,
          }
        : null,
      images: imageUrl.trim() ? [{ url: imageUrl.trim(), altText: name.trim(), sortOrder: 0 }] : [],
  ...
  return (
    <Card className="border-border/70 shadow-2xs text-xs">
      <CardHeader className="p-4 sm:p-5 pb-3 border-b border-border/60">
        <CardTitle className="text-sm sm:text-base font-bold">Product Information & Media</CardTitle>
        <CardDescription className="text-xs text-muted-foreground">
          Configure product title, category, manufacturer brand, ImageKit banner/gallery, and SEO.
        </CardDescription>
      </CardHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Input value={name} maxLength={200} onChange={(e) => handleNameChange(e.target.value)} placeholder="e.g. Nike Air Max 90" className="h-8.5 text-xs" required />
            </div>
            <div className="space-y-1.5">
              <label className="font-semibold text-foreground">URL Slug <span className="text-rose-500">*</span></label>
              <Input value={slug} maxLength={220} onChange={(e) => { setIsAutoSlug(false); setSlug(generateSlug(e.target.value)) }} placeholder="e.g. nike-air-max-90" className="h-8.5 text-xs font-mono" required />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <label className="font-semibold text-foreground">Brand</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center">
            <div className="space-y-1.5">
              ...
              <select value={status} onChange={(e) => setStatus(e.target.value as ProductStatus)} className="w-full h-8.5 rounded-md border border-input bg-background px-2.5 text-xs text-foreground focus:outline-hidden">
                <option value="DRAFT">DRAFT (Hidden from Store)</option>
                <option value="ACTIVE">ACTIVE (Published in Catalog)</option>
                <option value="ARCHIVED">ARCHIVED (Discontinued)</option>
              </select>
            </div>
            <div className="flex items-center justify-between p-2 rounded-lg border border-border/70 bg-muted/20">
              <div>
                <span className="font-semibold text-foreground block">Featured Showcase</span>
                <span className="text-[10.5px] text-muted-foreground">Highlight on homepage & deals</span>
              </div>
              <Switch checked={isFeatured} onCheckedChange={setIsFeatured} />
            </div>
          </div>

          {/* Media Links: Cover and ImageKit Hero Banner */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-1">
            <ImageUploadDropzone
              value={imageUrl}
              onChange={(url) => setImageUrl(url)}
              folder="/products"
              label="Primary Gallery Photo"
              description="Upload primary product photo from device (PNG, JPG, WebP)"
              aspectRatio="square"
            ...
            <ImageUploadDropzone
              ...
              description="Upload campaign banner from device (PNG, JPG, WebP)"
              aspectRatio="square"
              disabled={createMutation.isPending}
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea value={description} maxLength={10000} onChange={(e) => setDescription(e.target.value)} placeholder="Detailed specifications, features, warranty..." rows={3} className="w-full rounded-md border border-input bg-background p-2.5 text-xs text-foreground placeholder:text-muted-foreground focus:outline-hidden" />
          </div>
          ...
          <div className="border-t border-border/60 pt-3 space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input value={seoTitle} maxLength={70} onChange={(e) => setSeoTitle(e.target.value)} placeholder="e.g. Nike Air Max 90 | Official Store" className="h-8.5 text-xs" />
              </div>
              <div className="space-y-1.5">
                <label className="font-semibold text-foreground">SEO Description</label>
                <Input value={seoDescription} maxLength={160} onChange={(e) => setSeoDescription(e.target.value)} placeholder="e.g. Shop the legendary Nike Air Max 90." className="h-8.5 text-xs" />
              </div>
            </div>
          </div>

          <div className="flex items-center justify-end gap-2 pt-2 border-t border-border/60">
            <Button type="button" variant="outline" size="sm" onClick={() => router.push("/products")} disabled={createMutation.isPending} className="h-8.5 text-xs">
              Cancel
            </Button>
            <Button type="submit" size="sm" disabled={createMutation.isPending} className="h-8.5 gap-1.5 text-xs bg-indigo-600 hover:bg-indigo-700 text-white font-semibold">
